...
public class DownloadTask
        implements Callable<String>, Serializable, Cloneable
{
    ...
    private HttpClient getHttpClient()
    {
        ...
        HttpProtocolParams.setUserAgent(localBasicHttpParams, String.format("android-async-http/%s (http://loopj.com/android-async-http)", new Object[] { "1.0.0" }));
            ...
            throws Exception
    {
        ...
        RandomAccessFile localRandomAccessFile = new RandomAccessFile(getFilePath(), "rwd");
        ...
        while (true)
        {
            try
            {
                ...
                {
                    int k = NumUtils.computePercent(this.fileDownSize, this.fileTotalSize);
                    if (k - this.percent >= this.downloadManager.getNotifyFrequency())
                    {
                        this.percent = k;
                        if (this.lastTime == 0L)
            ...
            catch (Exception localException)
            {
                ...
                this.downloadManager.onUpdate(this);
                //continue;
                this.percent = NumUtils.computePercent(this.fileDownSize, this.fileTotalSize);
                if (this.lastTime == 0L)
                {
                    this.lastTime = System.currentTimeMillis();
                    continue;
        ...
        while (true)
        {
            label842: localInputStream.close();
            localRandomAccessFile.close();
            if (this.taskState != 4)
                break;
            this.downloadLogManager.updateTaskState(this);
