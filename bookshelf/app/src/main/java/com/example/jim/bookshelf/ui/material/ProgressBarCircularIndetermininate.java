...
public class ProgressBarCircularIndetermininate extends CustomView
{
    ...
    public ProgressBarCircularIndetermininate(Context paramContext, AttributeSet paramAttributeSet)
    {
    ...
    private void drawFirstAnimation(Canvas paramCanvas)
    {
        ...
        localPaint3.setColor(getResources().getColor(android.R.color.transparent));
        ...
        if (this.cont >= 50)
        {
            ...
            for (float f3 = getWidth() / 2.0F; ; f3 = 1.0F + this.radius2)
            {
                ...
                if (this.radius2 >= getWidth() / 2 - UIUtils.dpToPx(4.0F, getResources()))
        ...
        if (this.radius2 >= getWidth() / 2 - UIUtils.dpToPx(4.0F, getResources()));
        for (float f2 = getWidth() / 2.0F - UIUtils.dpToPx(4.0F, getResources()); ; f2 = 1.0F + this.radius2)
        {
            this.radius2 = f2;
            break;
        }
    ...
    private void drawSecondAnimation(Canvas paramCanvas)
    {
        ...
        Paint localPaint2 = new Paint();
        localPaint2.setAntiAlias(true);
        localPaint2.setColor(getResources().getColor(android.R.color.transparent));
        localPaint2.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
        localCanvas.drawCircle(getWidth() / 2, getHeight() / 2, getWidth() / 2 - UIUtils.dpToPx(4.0F, getResources()), localPaint2);
        paramCanvas.drawBitmap(localBitmap, 0.0F, 0.0F, new Paint());
    }
    ...
    {
        setMinimumHeight(UIUtils.dpToPx(32.0F, getResources()));
        setMinimumWidth(UIUtils.dpToPx(32.0F, getResources()));
        int i = paramAttributeSet.getAttributeResourceValue("http://schemas.android.com/apk/res/android", "background", -1);
        if (i != -1)
            setBackgroundColor(getResources().getColor(i));
        while (true)
        {
            setMinimumHeight(UIUtils.dpToPx(3.0F, getResources()));
